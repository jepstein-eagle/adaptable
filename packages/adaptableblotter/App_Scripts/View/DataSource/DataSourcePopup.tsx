...
interface DataSourcePopupProps extends StrategyViewPopupProps<DataSourcePopupComponent> {
    onAddUpdateDataSource: (Index: number, DataSource: IDataSource) => DataSourceRedux.DataSourceAddUpdateAction
    onChangeName: (DataSource: IDataSource, Name: string) => DataSourceRedux.DataSourceChangeNameAction
    onChangeDescription: (DataSource: IDataSource, Description: string) => DataSourceRedux.DataSourceChangeDescriptionAction
...
class DataSourcePopupComponent extends React.Component<DataSourcePopupProps, EditableConfigEntityState> {
    ...
    render() {
        ...
        let infoBody: any[] = ["Use DataSources to select from existing server queries what data to show in the Blotter."]
        ...
        let dataSources = this.props.DataSources.map((dataSource: IDataSource, index: number) => {
            return <DataSourceEntityRow
                ...
                AdaptableBlotterObject={dataSource} key={"ns" + index}
                Index={index}
                ...
                onChangeDescription={(dataSource, newOperation) => this.props.onChangeDescription(dataSource, newOperation)}
            >
            </DataSourceEntityRow>
        });
        ...
        return <div className={cssClassName}>
            <PanelWithButton cssClassName={cssClassName} headerText={StrategyConstants.DataSourceStrategyName} className="ab_main_popup"
                button={newButton}
                bsStyle="primary" glyphicon={StrategyConstants.DataSourceGlyph}
                infoBody={infoBody}>

                {dataSources.length > 0 ?
                    <AdaptableObjectCollection cssClassName={cssClassName} colItems={colItems} items={dataSources} />
                    :
    ...
    onFinishWizard() {
        ...
        let currentSearchIndex: number = this.props.DataSources.findIndex(as => as.Name == this.props.CurrentDataSource)
    ...
    canFinishWizard() {
        ...
        return StringExtensions.IsNotNullOrEmpty(DataSource.Name) && StringExtensions.IsNotNullOrEmpty(DataSource.Description)
    }

    CreateDataSource() {
        this.setState({ EditedAdaptableBlotterObject: ObjectFactory.CreateEmptyDataSource(), WizardStartIndex: 0 });
    }
...
function mapDispatchToProps(dispatch: Redux.Dispatch<AdaptableBlotterState>) {
    return {
        onAddUpdateDataSource: (Index: number, DataSource: IDataSource) => dispatch(DataSourceRedux.DataSourceAddUpdate(Index, DataSource)),
        onChangeName: (DataSource: IDataSource, Name: string) => dispatch(DataSourceRedux.DataSourceChangeName(DataSource, Name)),
        onChangeDescription: (DataSource: IDataSource, Description: string) => dispatch(DataSourceRedux.DataSourceChangeDescription(DataSource, Description)),
        onSelectDataSource: (SelectedDataSource: string)  => dispatch(DataSourceRedux.DataSourceSelect(SelectedDataSource)),
        onShare: (entity: IAdaptableBlotterObject) => dispatch(TeamSharingRedux.TeamSharingShare(entity, StrategyConstants.DataSourceStrategyId))
    };
}
