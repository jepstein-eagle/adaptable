...
export class DataSourceSettingsWizard extends React.Component<DataSourceSettingsWizardProps, DataSourceSettingsWizardState> implements AdaptableWizardStep {
                    ...
                    <FormGroup controlId="DataSourceName">
                        ...
                        <Col xs={7}>
                            <FormGroup controlId="formInlineName" validationState={validationState}>
                                <FormControl value={this.state.Name} type="string" placeholder="Enter DataSource name"
                    ...
                    <FormGroup controlId="DataSourceDescription">
                        ...
                        <Col xs={7}>
                            <FormGroup controlId="formInlineDescription" validationState={validationState}>
                                <FormControl value={this.state.Description} type="string" placeholder="Enter description"
    ...
    onDataSourceNameChange(event: React.FormEvent<any>) {
        ...
        this.setState({
            Name: e.value,
            ErrorMessage: ArrayExtensions.ContainsItem(this.props.DataSourceNames, e.value) ? "A data source already exists with that name" : null
        } as DataSourceSettingsWizardState, () => this.props.UpdateGoBackState())
    }

    onDataSourceDescriptionChange(event: React.FormEvent<any>) {
        let e = event.target as HTMLInputElement;
        this.setState({ Description: e.value, } as DataSourceSettingsWizardState, () => this.props.UpdateGoBackState())
    }


    public canNext(): boolean {
        return StringExtensions.IsNotNullOrEmpty(this.state.Name) && StringExtensions.IsNotNullOrEmpty(this.state.Description)
    }
