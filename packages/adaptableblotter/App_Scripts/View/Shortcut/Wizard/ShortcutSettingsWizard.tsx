...
export class ShortcutSettingsWizard
            ...
            <FormGroup controlId="formInlineKey">
              ...
              <Col xs={6}>
                <FormControl
                  ...
                  onChange={x => this.onShortcutKeyChanged(x)}
                >
                ...
                <FormGroup controlId="formInlineAction">
                  ...
                  <Col xs={6}>
                    <FormControl
                      ...
                      onChange={x => this.onShortcutOperationChanged(x)}
                    >
                      {optionActions}
                    </FormControl>
                  </Col>
                ...
                <FormGroup controlId="formInlineDateType">
                  ...
                  <Col xs={6}>
                    <Radio
                      ...
                      onChange={e => this.onDynamicSelectChanged(e)}
                    >
                      Custom
                    </Radio>
                    <Radio
                  ...
                  <FormGroup controlId="formInlineDateResultPredefined">
                    ...
                    <Col xs={6}>
                      <FormControl
                        ...
                        onChange={x => this.onDynamicResultChanged(x)}
                      >
                        <option value="select" key="select">
                          Select Dynamic Date
                        </option>
                        <option value={CalendarConstants.TODAY} key={CalendarConstants.TODAY}>
                          Today
                        </option>
                        <option
                          value={CalendarConstants.YESTERDAY}
                          key={CalendarConstants.YESTERDAY}
                        >
                          Yesterday
                        </option>
                        <option value={CalendarConstants.TOMORROW} key={CalendarConstants.TOMORROW}>
                          Tomorrow
                        </option>
                        <option
                          value={CalendarConstants.PREVIOUS_WORK_DAY}
                          key={CalendarConstants.PREVIOUS_WORK_DAY}
                        >
                          Previous Work Day
                        </option>
                        <option
                          value={CalendarConstants.NEXT_WORK_DAY}
                          key={CalendarConstants.NEXT_WORK_DAY}
                        >
                          Next Work Day
                        </option>
                      </FormControl>
  ...
  private onShortcutKeyChanged(event: React.FormEvent<any>) {
    ...
    this.setState({ ShortcutKey: e.value } as ShortcutSettingsWizardState, () =>
      this.props.UpdateGoBackState()
    );
  }
  ...
  private onShortcutOperationChanged(event: React.FormEvent<any>) {
    let e = event.target as HTMLInputElement;
    this.setState({ ShortcutOperation: e.value } as ShortcutSettingsWizardState, () =>
      this.props.UpdateGoBackState()
    );
  }
  ...
  private onDynamicResultChanged(event: React.FormEvent<any>) {
    let e = event.target as HTMLInputElement;
    this.setState({ ShortcutResult: e.value } as ShortcutSettingsWizardState, () =>
      this.props.UpdateGoBackState()
    );
  }
