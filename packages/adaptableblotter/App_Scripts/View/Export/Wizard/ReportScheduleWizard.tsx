...
export class ReportScheduleWizard extends React.Component<ReportScheduleWizardProps, ReportScheduleWizardState> implements AdaptableWizardStep {
    constructor(props: ReportScheduleWizardProps) {
        super(props);
        let autoExport: IAutoExport = (this.props.Data.AutoExport) ? this.props.Data.AutoExport : ObjectFactory.CreateEmptyAutoExport();
                    ...
                    <div style={{ marginTop: '30px' }}>
                            ...
                            <Row>
                                ...
                                <Col xs={6}>
                                    <FormControl
                                        componentClass="select"
                                        placeholder="select"
                                        value={this.state.Minute} onChange={(x) => this.onMinuteChanged(x)} >
                                        {minutes}
                                    </FormControl>
                                </Col>
                            </Row>
                        ...
                        {this.state.IsRecurringDate ?
                            <Row><Col xs={3} />
                                <Col xs={6}>
                                    <Panel>
                                        <Checkbox className={cssClassName + "__checkbox"} inline value={DayOfWeek.Monday} checked={ArrayExtensions.ContainsItem(this.state.DaysOfWeek, DayOfWeek.Monday)} onChange={(e) => this.onDayChecked(e)}>Monday</Checkbox><br />
                                        <Checkbox className={cssClassName + "__checkbox"} inline value={DayOfWeek.Tuesday} checked={ArrayExtensions.ContainsItem(this.state.DaysOfWeek, DayOfWeek.Tuesday)} onChange={(e) => this.onDayChecked(e)}>Tuesday</Checkbox><br />
    ...
    private onHasAutoExportChanged(event: React.FormEvent<any>) {
        ...
        if (e.checked) {
            this.setState({ HasAutoExport: true } as ReportScheduleWizardState, () => this.props.UpdateGoBackState())
        } else {
            this.setState({ HasAutoExport: false } as ReportScheduleWizardState, () => this.props.UpdateGoBackState())
        }
    }
    ...
    private onDayChecked(event: React.FormEvent<any>) {
        ...
        this.setState({ DaysOfWeek: daysOfWeek } as ReportScheduleWizardState, () => this.props.UpdateGoBackState())
    }
    ...
    private onRecurringDateChanged(event: React.FormEvent<any>) {
        ...
        this.setState({ IsRecurringDate: (e.value == "recurring") } as ReportScheduleWizardState, () => this.props.UpdateGoBackState())
    }

    private onHourChanged(event: React.FormEvent<any>) {
        let e = event.target as HTMLInputElement;
        this.setState({ Hour: Number(e.value) } as ReportScheduleWizardState, () => this.props.UpdateGoBackState())
    }

    private onMinuteChanged(event: React.FormEvent<any>) {
        let e = event.target as HTMLInputElement;
        this.setState({ Minute: Number(e.value) } as ReportScheduleWizardState, () => this.props.UpdateGoBackState())
    }
