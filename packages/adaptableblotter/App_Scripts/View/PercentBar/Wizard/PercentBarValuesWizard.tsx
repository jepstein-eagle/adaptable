...
export class PercentBarValuesWizard extends React.Component<PercentBarValuesWizardProps, PercentBarValuesWizardState> implements AdaptableWizardStep {
                ...
                <AdaptableBlotterForm >
                    ...
                    <FormGroup controlId="formMinimumValueSelect">
                        <Row>
                            ...
                            <Col xs={6} >
                                <Radio inline value="value" checked={this.state.UseMinColumn == false} onChange={(e) => this.onUseMinColumnSelectChanged(e)}>Static Value</Radio>
                                {' '}
                ...
                <AdaptableBlotterForm >
                    ...
                    <FormGroup controlId="formMaximumValueSelect">
                        <Row>
                            ...
                            <Col xs={6} >
                                <Radio inline value="value" checked={this.state.UseMaxColumn == false} onChange={(e) => this.onUseMaxColumnSelectChanged(e)}>Static Value</Radio>
                                {' '}
    ...
    private onUseMinColumnSelectChanged(event: React.FormEvent<any>) {
        ...
        this.setState({ UseMinColumn: (e.value == "column") } as PercentBarValuesWizardState, () => this.props.UpdateGoBackState())
    }

    private onMinValueChanged = (e: any) => {
        this.setState({ MinValue: e.target.value } as PercentBarValuesWizardState, () => this.props.UpdateGoBackState())
    }

    private onColumnMinValueSelectedChanged(columns: IColumn[]) {
        this.setState({ MinValueColumnId: columns.length > 0 ? columns[0].ColumnId : "" } as PercentBarValuesWizardState, () => this.props.UpdateGoBackState())
    }

    private onUseMaxColumnSelectChanged(event: React.FormEvent<any>) {
        let e = event.target as HTMLInputElement;
        this.setState({ UseMaxColumn: (e.value == "column") } as PercentBarValuesWizardState, () => this.props.UpdateGoBackState())
    }

    private onMaxValueChanged = (e: any) => {
        this.setState({ MaxValue: e.target.value } as PercentBarValuesWizardState, () => this.props.UpdateGoBackState())
    }

    private onColumnMaxValueSelectedChanged(columns: IColumn[]) {
        this.setState({ MaxValueColumnId: columns.length > 0 ? columns[0].ColumnId : "" } as PercentBarValuesWizardState, () => this.props.UpdateGoBackState())
    }
