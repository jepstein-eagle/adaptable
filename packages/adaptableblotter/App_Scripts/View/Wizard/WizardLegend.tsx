...
export class WizardLegend extends React.Component<WizardLegendProps, {}> {
    render(): any {
        ...
        let stepNames: any = this.props.StepNames.map((s, index) => {
            ...
            let isDisabled: boolean = isActiveStep || index > activeStepIndex;
            let style: string = isActiveStep? "primary" : "default";

            let lastStep: boolean = (index == count);
            return <span key={index}>
                <Button bsStyle={style} bsSize={"small"} disabled={isDisabled} onClick={()=>this.onStepButtonClicked(s)} >{s}</Button>
                {lastStep == false &&
