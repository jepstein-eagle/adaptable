...
export class ReminderAlertWizard extends React.Component<ReminderAlertWizardProps, ReminderAlertWizardState> implements AdaptableWizardStep {
            ...
            <Panel header="Alert Settings" bsStyle="primary">
                <AdaptableBlotterForm inline>
                    <Row style={{ marginTop: '15px' }}>
                        ...
                        <Col xs={8}>
                            <FormControl value={this.state.Header} style={{width:'100%'}} type="string" placeholder="Enter Reminder Header (optional)" onChange={(e) => this.onHeaderChanged(e)} />
                        </Col>
                    </Row>
                    <Row style={{ marginTop: '15px' }}>
                        <Col xs={3}>
                        ...
                        <Col xs={8}>
                            <FormControl value={this.state.Msg} style={{width:'100%'}} type="string" placeholder="Enter Reminder Message" onChange={(e) => this.onMessageChanged(e)} />
                    ...
                    <Row style={{ marginTop: '15px' }}>
                        ...
                        <Col xs={6}>
                            <FormControl componentClass="select" placeholder="select" value={this.state.MessageType} onChange={(x) => this.onMessageTypeChanged(x)} >
                                {messageTypes}
                            </FormControl>
                        </Col>
                    </Row>
                    <Row style={{ marginTop: '15px' }}>
                        ...
                        <Col xs={6}>
                            ...
                            {' '} {' '}
                            <AdaptablePopover cssClassName={cssClassName} headerText={"Reminder Details"} bodyText={["A popup is displayed when the Reminder is triggered."]} />
                        </Col>
                    </Row>
    ...
    private onHeaderChanged(event: React.FormEvent<any>) {
        ...
        this.setState({ Header: e.value } as ReminderAlertWizardState, () => this.props.UpdateGoBackState())
    }

    private onMessageChanged(event: React.FormEvent<any>) {
        let e = event.target as HTMLInputElement;
        this.setState({ Msg: e.value } as ReminderAlertWizardState, () => this.props.UpdateGoBackState())
    }

    private onMessageTypeChanged(event: React.FormEvent<any>) {
        let e = event.target as HTMLInputElement;
        this.setState({ MessageType: e.value } as ReminderAlertWizardState, () => this.props.UpdateGoBackState())
    }

    private onShowAsPopupChanged(event: React.FormEvent<any>) {
        let e = event.target as HTMLInputElement;
        this.setState({ ShowAsPopup: e.checked } as ReminderAlertWizardState, () => this.props.UpdateGoBackState())
    }
