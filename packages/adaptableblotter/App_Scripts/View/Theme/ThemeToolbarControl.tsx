...
class ThemeToolbarControlComponent extends React.Component<ThemeToolbarControlComponentProps, {}> {
    ...
    render(): any {
        ...
        let themes: any[] = allThemes.filter(s => s != this.props.CurrentTheme).map((search, index) => {
            return <MenuItem key={index} eventKey={index} onClick={() => this.onSelectTheme(search)} >{search}</MenuItem>
        })

        let content = <div className={this.props.AccessLevel == AccessLevel.ReadOnly ? GeneralConstants.READ_ONLY_STYLE : ""}>
...
export let ThemeToolbarControl = connect(mapStateToProps, mapDispatchToProps)(ThemeToolbarControlComponent);