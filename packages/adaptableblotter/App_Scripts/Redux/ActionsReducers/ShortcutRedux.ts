...
export const ShortcutChangeKey = (
  Shortcut: IShortcut,
  NewShortcutKey: string
): ShortcutChangeKeyAction => ({
  type: SHORTCUT_CHANGE_KEY,
  Shortcut,
  NewShortcutKey,
});

export const ShortcutChangeOperation = (
  Shortcut: IShortcut,
  NewShortcutOperation: MathOperation
): ShortcutChangeOperationAction => ({
  type: SHORTCUT_CHANGE_OPERATION,
  Shortcut,
  NewShortcutOperation,
});

export const ShortcutChangeResult = (
  Shortcut: IShortcut,
  NewShortcutResult: any
): ShortcutChangeResultAction => ({
...
export const ShortcutReducer: Redux.Reducer<ShortcutState> = (
  ...
  switch (action.type) {
    ...
    case SHORTCUT_CHANGE_OPERATION: {
      ...
      shortcuts = [].concat(state.Shortcuts);
      let index = shortcuts.indexOf(shortcut);
      shortcuts[index] = Object.assign({}, shortcut, {
        ShortcutOperation: actionTyped.NewShortcutOperation,
      });
      return Object.assign({}, state, { Shortcuts: shortcuts });
    }
    case SHORTCUT_CHANGE_RESULT: {
      ...
      let index = shortcuts.indexOf(shortcut);
      shortcuts[index] = Object.assign({}, shortcut, {
        ShortcutResult: actionTyped.NewShortcutResult,
      });
      return Object.assign({}, state, { Shortcuts: shortcuts });
    }
    case SHORTCUT_ADD: {
