...
export interface StateOptions {
  /**
   * Allows the customization of state persistence. By default, state stringified using JSON.stringify and persisted into the localStorage, at the key denoted by `adaptableId` - in case no config server was defined via `configServerOptions`. If a config server was specified, the default implementation sends a POST request at the url of the config server, with the adaptable state as the body (in JSON) of the request.
   *
   ...
   *
   * For example, `loadState` could be used to load the adaptable state from the browser window object or from a remote location altogether.
   *
   ...
   *
   * NOTE: If you used `saveState` to add custom app-specific properties on the top-level object you returned, it's a good practice to clear these properties and just return the adaptable state from the `applyState` function.
   *
