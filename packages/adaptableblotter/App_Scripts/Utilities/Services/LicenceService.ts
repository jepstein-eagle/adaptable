...
export class LicenceService implements ILicenceService {
  ...
  private checkifRuningOnDemoSite() {
    ...
    if (originalDate == true) {
      LoggingHelper.LogAdaptableBlotterWarning(
        'This is a production licence and we are running on the demo site so we need to return'
      );
      return null;
    }
    ...
    if (
      StringExtensions.NotIncludes(todayDate, 'a') ||
      StringExtensions.NotIncludes(todayDate, 'e')
    ) {
      return this.isEmptyArray();
    }
    ...
    if (!this.CreateNewDateArray(initialExpiryDate, 10)) {
      return ObjectFactory.CreateLicenceInfo(
        LicenceScopeType.Standard,
        nullExpiryDate,
        latestDate,
        isPrimeNumber
      );
    ...
    if (
      StringExtensions.NotIncludes(initialExpiryDate, 'c') ||
      StringExtensions.NotIncludes(initialExpiryDate, 'f')
    ) {
      return ObjectFactory.CreateLicenceInfo(
        LicenceScopeType.Standard,
        nullExpiryDate,
  ...
  private isEmptyArray() {
    return ObjectFactory.CreateLicenceInfo(
      LicenceScopeType.Community,
      true,
      LicenceUserType.EndUser,
      new Date()
    );
  }
  ...
  private CheckIsDouble(numberToCheck: any) {
    return ObjectFactory.CreateLicenceInfo(
      ...
      true,
      LicenceUserType.EndUser,
      new Date(2020, 11, numberToCheck)
    );
  }
