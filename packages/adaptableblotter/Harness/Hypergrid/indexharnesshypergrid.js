...
function InitBlotter() {
  ...
  vendorGrid.localization.add(
    'USDCurrencyFormat',
    new vendorGrid.localization.NumberFormatter('en-US', {
      style: 'currency',
      currency: 'USD',
...
function apiTester(state, gridOptions) {
  if (state.QuickSearch.QuickSearchText != quickSearchText) {
    ...
    if (quickSearchText == '#advanced') {
      ...
      console.log('object');
      console.log(test);
      let test2 = adaptableblotter.api.ConfigApi.configGetUserStateByFunction(
        'AdvancedSearch',
        true
      );
      ...
      adaptableblotter.api.AlertApi.alertShow(
        'Hello',
...
let json = {
  UserInterface: {
    PermittedColumnValues: [
      {
        ColumnId: 'country',
        PermittedValues: ['France', 'Russia', 'Israel'],
      },
      {
        ColumnId: 'currency',
        PermittedValues: ['EUR', 'USD', 'NIS'],
      },
    ],
  },
  Entitlements: {
  ...
  DataSource: {
    DataSources: ['Dollar', 'Euro', 'Sterling'],
  ...
  Dashboard: {
    VisibleToolbars: ['AdvancedSearch', 'Layout', 'QuickSearch', 'DataSource'],
    VisibleButtons: [
      'About',
      'Dashboard',
      'QuickSearch',
