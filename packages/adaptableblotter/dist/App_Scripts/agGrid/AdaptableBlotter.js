...
class AdaptableBlotter {
    ...
    getQuickSearchClassName() {
        let quickSearchClassName = StringExtensions_1.StringExtensions.IsNotNullOrEmpty(this.api.quickSearchApi.getQuickSearchStyle().ClassName) ?
    ...
    setValueBatch(batchValues) {
        ...
        let percentBars = this.api.percentBarApi.getAllPercentBar();
    ...
    setCellClassRules(cellClassRules, columnId, type) {
        ...
        if (localCellClassRules) {
            if (type == "FormatColumn") {
                ...
                let cssStyles = this.api.conditionalStyleApi.getAllConditionalStyle().map(c => c.Style.ClassName);
                for (let prop in localCellClassRules) {
    ...
    addCalculatedColumnToGrid(calculatedColumn) {
        ...
        let newColDef = {
            ...
            valueGetter: (params) => Helper_1.Helper.RoundValueIfNumeric(this.CalculatedColumnExpressionService.ComputeExpressionValue(cleanedExpression, params.node), 4)
        };
    ...
    initInternalGridLogic() {
        ...
        this.gridOptions.api.addEventListener(eventKeys_1.Events.EVENT_FIRST_DATA_RENDERED, () => {
            this.debouncedSetColumnIntoStore();
        });
        // once the grid is ready we should make sure we are too
        this.gridOptions.api.addEventListener(eventKeys_1.Events.EVENT_GRID_READY, () => {
            // do something?
        });
        ...
        this.gridOptions.api.addEventListener(eventKeys_1.Events.EVENT_CELL_EDITING_STOPPED, () => {
        ...
        //     console.log(params)
        // });
        this.gridOptions.api.addEventListener(eventKeys_1.Events.EVENT_SORT_CHANGED, () => {
            this.onSortChanged();
            this.debouncedSetSelectedCells();
        });
        ...
        this.gridOptions.doesExternalFilterPass = (node) => {
            ...
            if (this.blotterOptions.generalOptions.serverSearchOption == 'None' || this.blotterOptions.generalOptions.serverSearchOption == 'AdvancedSearch') {
                let columnFilters = this.api.columnFilterApi.getAllColumnFilter();
                ...
                let quickSearchState = this.api.quickSearchApi.getQuickSearchState();
...
*/
    ...
    addPercentBar(pcr) {
        ...
        if (renderedColumn) {
            let cellRendererFunc = this.agGridHelper.createCellRendererFunc(pcr, this.blotterOptions.blotterId);
            ...
            if (sortModel.length > 0) {
                sortModel.forEach(sm => {
                    if (ColumnHelper_1.ColumnHelper.isSpecialColumn(sm.colId)) {
                        ...
                        if (groupedColumn) {
                            let customSort = this.api.customSortApi.getAllCustomSort().find(cs => cs.ColumnId == groupedColumn.getColId());
                            if (customSort) {
    ...
    applyFinalRendering() {
        ...
        let currentlayout = this.api.layoutApi.getCurrentLayoutName();
        if (currentlayout != GeneralConstants_1.DEFAULT_LAYOUT && ArrayExtensions_1.ArrayExtensions.IsNotNullOrEmpty(this.api.percentBarApi.getAllPercentBar())) {
