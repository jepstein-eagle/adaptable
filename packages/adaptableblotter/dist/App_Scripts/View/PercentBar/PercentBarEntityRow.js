...
class PercentBarEntityRow extends React.Component {
    render() {
        ...
        colItems[1].Content = React.createElement(EntityRowItem_1.EntityRowItem, { Content: (StringExtensions_1.StringExtensions.IsNullOrEmpty(PercentBar.MinValueColumnId)) ?
                React.createElement(react_bootstrap_1.FormControl, { bsSize: 'small', type: "number", placeholder: "Min Value", onChange: (e) => this.onMinimumValueChanged(e), value: PercentBar.MinValue })
                :
                    "[" + ColumnHelper_1.ColumnHelper.getFriendlyNameFromColumnId(PercentBar.MinValueColumnId, this.props.Columns) + "]" });
        colItems[2].Content = React.createElement(EntityRowItem_1.EntityRowItem, { Content: (StringExtensions_1.StringExtensions.IsNullOrEmpty(PercentBar.MaxValueColumnId)) ?
                React.createElement(react_bootstrap_1.FormControl, { bsSize: "small", type: "number", placeholder: "Max Value", onChange: (e) => this.onMaximumValueChanged(e), value: PercentBar.MaxValue })
                :
                    "[" + ColumnHelper_1.ColumnHelper.getFriendlyNameFromColumnId(PercentBar.MaxValueColumnId, this.props.Columns) + "]" });
        colItems[3].Content = React.createElement(EntityRowItem_1.EntityRowItem, { Content: React.createElement(ColorPicker_1.ColorPicker, { ColorPalette: this.props.ColorPalette, value: PercentBar.PositiveColor, onChange: (x) => this.onPositiveColorChanged(x) }) });
        colItems[4].Content = React.createElement(EntityRowItem_1.EntityRowItem, { Content: React.createElement(ColorPicker_1.ColorPicker, { ColorPalette: this.props.ColorPalette, value: PercentBar.NegativeColor, onChange: (x) => this.onNegativeColorChanged(x) }) });
