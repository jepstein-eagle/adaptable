...
class CellValidationRulesWizard extends React.Component {
    ...
    createCellValidationDescription(CellValidation) {
        let valueDescription = ExpressionHelper_1.ExpressionHelper.OperatorToLongFriendlyString(CellValidation.Range.Operator, this.getColumnDataTypeFromState());
        if (!this.operatorRequiresValue(CellValidation.Range.Operator)) {
            return valueDescription;
        }
        let dataType = this.props.Columns.find(c => c.ColumnId == CellValidation.ColumnId).DataType;
        ...
        if (CellValidation.Range.Operator == Enums_1.LeafExpressionOperator.PercentChange) {
            valueDescription = valueDescription + '%';
        }
        if (StringExtensions_1.StringExtensions.IsNotNullOrEmpty(CellValidation.Range.Operand2)) {
