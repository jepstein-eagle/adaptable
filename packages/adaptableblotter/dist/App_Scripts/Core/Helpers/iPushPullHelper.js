...
(function (iPushPullHelper) {
    ...
    function GetDomainPages(clientId) {
        return new Promise((resolve, reject) => {
            ...
            serv2.getDomainsAndPages(clientId)
                ...
                .catch((x) => {
                AdaptableBlotterLogger_1.AdaptableBlotterLogger.LogError("couldn't get Domain/Pages from IPP : ", x);
                reject(x.message);
            });
        });
    }
    ...
    function LoadPage(folderIPP, pageIPP) {
        return new Promise((resolve, reject) => {
            ...
            page.on(page.EVENT_NEW_CONTENT, function (data) {
                AdaptableBlotterLogger_1.AdaptableBlotterLogger.LogMessage("Page Ready : " + pageIPP);
    ...
    function UnloadPage(page) {
        ...
        if (pageIPP) {
            pageIPP.destroy();
            pages.delete(page);
            AdaptableBlotterLogger_1.AdaptableBlotterLogger.LogMessage("Page Unloaded : " + page);
        }
    }
    ...
    function pushData(page, data, style) {
        return new Promise((resolve, reject) => {
            ...
            pageIPP.push().then(function () {
                AdaptableBlotterLogger_1.AdaptableBlotterLogger.LogMessage('Data pushed for page : ' + page);
                resolve();
            }, (err) => {
                AdaptableBlotterLogger_1.AdaptableBlotterLogger.LogMessage('Error pushing Data for page : ' + page);
                reject();
            });
        });
    }
