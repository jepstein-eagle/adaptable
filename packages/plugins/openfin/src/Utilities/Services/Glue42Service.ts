...
import { IOpenFinService, IOpenFinService } from './Interface/IOpenFinService';
...
export class OpenFinService implements IOpenFinService {
  ...
  constructor(private adaptable: IAdaptable, options: OpenFinPluginOptions) {
    ...
    this.adaptable.api.eventApi.on('AdaptableReady', () => {
      ...
      if (!this.glueInstance) {
        let OpenFinState: OpenFinState | undefined = OpenFinApi.getOpenFinState();
  ...
  async login(username: string, password: string, gatewayURL: string): Promise<void> {
    try {
      ...
      if (OpenFinConfig && OpenFinConfig.excelExport && OpenFinConfig.excelExport.timeoutMs) {
        this.excelSyncTimeout = OpenFinConfig.excelExport.timeoutMs;
      }

      this.glueInstance = await glue(OpenFinConfig.initialization);
      // Avoid a circular assignment:
      const glue4OfficeConfig: any = cloneDeep(OpenFinConfig.initialization);
      glue4OfficeConfig.glue = this.glueInstance;

      const glue4OfficeInstance = await glue4Office(glue4OfficeConfig);
      this.glue4ExcelInstance = glue4OfficeInstance.excel; // as OpenFinOffice.Excel.API;
      this.subscribeToAddinStatusChanges();
      this.getOpenFinApi().setOpenFinRunningOn();
    } catch (error) {
      LogAdaptableError(error);
      this.getOpenFinApi().setOpenFinRunningOff();
    }
  }
  ...
  async exportData(data: any[], gridColumns: AdaptableColumn[], primaryKeys: any[]) {
    ...
    try {
      ...
      const syncOptions: any = {
        workbook: 'OpenFin Excel Integration Demo',
  ...
  private getSheetChangeHandler(
    ...
    return (
      ...
      const errors: OpenFinExportError[] = [];
  ...
  createColumns(data: any[]): OpenFinColumnInfo[] {
    let firstRow: string[] = data[0];
    let headers: OpenFinColumnInfo[] = [];
