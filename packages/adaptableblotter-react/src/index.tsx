...
type TypeFactory = string | typeof React.Component | (({ children }: { children: ReactNode }) => ReactNode)
...
const getAgGridWrapperClassName = (agGridTheme: string) => {
  const themeClassName = agGridTheme.indexOf('ag-theme') === 0 ? agGridTheme : `ag-theme-${agGridTheme}`
...
const AdaptableBlotterReact = ({
  ...
  const gridWrapperNode = <DefiniteHeight
    key="agGridWrapper"
    style={{ flex: 1 }}
    childProps={{
