...
export class CellValidationStrategy extends AdaptableStrategyBase implements ICellValidationStrategy {
    ...
    private IsFailedValidation(cellValidationRule: ICellValidationRule, dataChangedEvent: IDataChangedEvent, columns: IColumn[]): boolean {
        // first see if the Expression has passed (if there is one)
        if (cellValidationRule.HasOtherExpression) {
            let isExpressionValid = ExpressionHelper.checkForExpression(cellValidationRule.OtherExpression, dataChangedEvent.IdentifierValue, columns, this.blotter);
    ...
    private getExpressionDescription(validationRule: ICellValidationRule, columns: IColumn[]): string {
